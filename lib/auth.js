    ...
    restler.get(url, {query: {token: Rhizome.authToken}})
      .on('success', data => {
        if (data === false) {
          ...
          url = `${Rhizome.url}/user/${userId}/${user.app}/info`;
          restler.put(url, {query: {token: Rhizome.authToken}, data: user})
            .on('error', err => {
              throw err;
              // reject(err);
            });
...
const _addAuthToUser = (userId, appAuth) => {
  return new Promise((resolve, reject) => {
    let url = `${Rhizome.url}/user/${userId}/auth`;
    console.log(url);
    restler.put(url, {query: {token: Rhizome.authToken}, data: {auth: appAuth}})
      .on('success', user => {
        console.log(user);
        resolve(Object.assign(user, {
          rhizomeId: userId,
          rhizomeAuthToken: user.authToken
        }));
      })
      .on('error', err => {
        console.error(err);
